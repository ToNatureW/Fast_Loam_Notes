...
void LaserProcessingClass::featureExtraction(const pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_in, pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_out_edge, pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_out_surf){
    ...
    pcl::removeNaNFromPointCloud(*pc_in, indices);      // 去除一帧点云中的无效点


    int N_SCANS = lidar_param.num_lines;
    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> laserCloudScans;  // 激光雷达每个线束对应的点云数组
    for(int i=0;i<N_SCANS;i++){
        laserCloudScans.push_back(pcl::PointCloud<pcl::PointXYZI>::Ptr(new pcl::PointCloud<pcl::PointXYZI>()));
    }


    //计算每个点与雷达中心和地面的夹角，并分到对应线束中　
    for (int i = 0; i < (int) pc_in->points.size(); i++)
    {
        int scanID=0;
        double distance = sqrt(pc_in->points[i].x * pc_in->points[i].x + pc_in->points[i].y * pc_in->points[i].y);  //判断是否在激光雷达点云范围阈值内
    ...
    for(int i = 0; i < N_SCANS; i++){
        if(laserCloudScans[i]->points.size()<131){      //线束中点云较少不进行曲率提取，并划分角点和面点
            continue;
        }
        
        std::vector<Double2d> cloudCurvature;  //　线束中点的曲率数数组
        ...
        for(int j = 5; j < (int)laserCloudScans[i]->points.size() - 5; j++){
            ...
            Double2d distance(j,diffX * diffX + diffY * diffY + diffZ * diffZ);  //Double2d的实例化，不知道为啥起这么个名字
        ...
        for(int j=0;j<6;j++){
            ...
            std::vector<Double2d> subCloudCurvature(cloudCurvature.begin()+sector_start,cloudCurvature.begin()+sector_end);     // 将每一线束划分为六部分，分别找到对应的始终之间的曲率值
...
/**
 *根据每个线束划分的六部分，选取角点，并得到面点
 * @param pc_in
 * @param cloudCurvature  // 将每一线束划分为六部分，分别找到对应的始终之间的曲率值
 * @param pc_out_edge
 * @param pc_out_surf
 */
void LaserProcessingClass::featureExtractionFromSector(const pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_in, std::vector<Double2d>& cloudCurvature, pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_out_edge, pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_out_surf){
    ...
    std::vector<int> picked_points; // 处理过的特征点的索引
    int point_info_count =0;
    for (int i = cloudCurvature.size()-1; i >= 0; i--)
    {
        int ind = cloudCurvature[i].id; 
        if(std::find(picked_points.begin(), picked_points.end(), ind)==picked_points.end()){   // 判断是否已经取过这个id点
            if(cloudCurvature[i].value <= 0.1){         // 如果曲率太小，不再继续取点
                break;
            }
            ...
            if (largestPickedNum <= 20){
                pc_out_edge->push_back(pc_in->points[ind]);     //　角点：曲率较大的点
                point_info_count++;
            }else{
                break;
            }

            // 当前点周围的十个点如果变化距离小于0.05则，不选取作为角点
            for(int k=1;k<=5;k++){
    ...
    for (int i = 0; i <= (int)cloudCurvature.size()-1; i++)
    {
        ...
        if( std::find(picked_points.begin(), picked_points.end(), ind)==picked_points.end())
        {
            pc_out_surf->push_back(pc_in->points[ind]);     //　进行选角点后，其他的点都是面点
        }
    }
}
